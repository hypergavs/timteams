...
<script>
  export default {
    ...
    methods: {
      ...
      editItem (item) {
        this.editedIndex = this.timPlayers.indexOf(item);
      ...
      deleteItem (item) {
        const index = this.timPlayers.indexOf(item);
        if(confirm('Are you sure you want to delete this item?')){
          this.$store.dispatch('players/deletePlayer', item.id);
          this.timPlayers.splice(index, 1);
        }
      },
      ...
      save () {
        if(this.editedIndex!==-1){
          this.$store.dispatch('players/updatePlayer', this.editedItem)
          .then(()=>{
            this.editedIndex = -1;
            this.dialog = false;
          })
        }else{
          this.$store.dispatch('players/addPlayer', this.editedItem)
            .then(()=>{
              this.dialog = false;
            })
        }
